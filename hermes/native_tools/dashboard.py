...
<main class="shell">
  ...
  <section id="cabinet" hidden>
    ...
    <section id="view-today" class="view">
      ...
      <button id="architecture-open-home" class="architecture-teaser" type="button" aria-haspopup="dialog"><span><span class="eyebrow">КАК ЭТО РАБОТАЕТ</span><strong>От сообщения до результата</strong><small>8 шагов: Telegram, Codex, память, инструменты и итог.</small></span><span class="architecture-teaser-action">Карта</span></button>
...
<dialog id="architecture-dialog"><form method="dialog" class="architecture-sheet"><p class="eyebrow">КАРТА СИСТЕМЫ</p><h2>JarHert · как работает</h2><p class="muted">Нажми шаг: покажу, что происходит с запросом и где ты сохраняешь контроль.</p><div class="architecture-flow" aria-label="Шаги работы JarHert"><button class="architecture-step is-active" data-architecture-step="telegram" type="button" aria-pressed="true"><span>1</span><strong>Telegram</strong></button><button class="architecture-step" data-architecture-step="codex" type="button" aria-pressed="false"><span>2</span><strong>Codex</strong></button><button class="architecture-step" data-architecture-step="tools" type="button" aria-pressed="false"><span>3</span><strong>Инструменты</strong></button><button class="architecture-step" data-architecture-step="memory" type="button" aria-pressed="false"><span>4</span><strong>Память</strong></button><button class="architecture-step" data-architecture-step="integrations" type="button" aria-pressed="false"><span>5</span><strong>Сервисы</strong></button><button class="architecture-step" data-architecture-step="plan" type="button" aria-pressed="false"><span>6</span><strong>План</strong></button><button class="architecture-step" data-architecture-step="radar" type="button" aria-pressed="false"><span>7</span><strong>Радар</strong></button><button class="architecture-step" data-architecture-step="code" type="button" aria-pressed="false"><span>8</span><strong>Код</strong></button></div><article class="architecture-detail" aria-live="polite"><p id="architecture-detail-eyebrow" class="eyebrow">ШАГ 1 · ВХОД</p><h3 id="architecture-detail-title">Telegram — единственная входная дверь</h3><p id="architecture-detail-copy">Текст, голос, кнопка или файл приходят в один Hermes gateway. Параллельный старый бот не обрабатывает те же сообщения.</p><p id="architecture-detail-guard" class="muted">Голос сначала превращается в текст, затем ясные пункты собираются в один план.</p></article><div class="dialog-actions"><button class="primary" type="submit">Понятно</button></div></form></dialog>

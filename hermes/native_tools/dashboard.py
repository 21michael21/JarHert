...
<dialog id="architecture-dialog"><form method="dialog" class="architecture-sheet"><p class="eyebrow">КАРТА СИСТЕМЫ</p><h2>JarHert · как работает</h2><p class="muted">Один личный контур: Telegram остаётся входом, а данные и действия не размазываются по сервисам.</p><div class="architecture-flow"><article class="architecture-step"><span>1</span><div><strong>Telegram или голос</strong><p>Сообщение приходит в единственный Hermes gateway. Голос сначала превращается в текст локально.</p></div></article><article class="architecture-step"><span>2</span><div><strong>Codex backend + SOUL</strong><p>Hermes понимает запрос по-русски, выбирает короткий ответ или конкретное действие. Стиль и память не смешиваются с сырыми чатами.</p></div></article><article class="architecture-step"><span>3</span><div><strong>Узкий набор инструментов</strong><p>Для вопроса открывается только нужный набор: личное, планирование, исследование или код.</p></div></article><article class="architecture-step"><span>4</span><div><strong>Personal OS</strong><p>Заметки, проекты, контакты, обещания и напоминания хранятся в одной SQLite-базе с поиском.</p></div></article><article class="architecture-step"><span>5</span><div><strong>Внешние действия</strong><p>Trello и Calendar идут через Task Command Center. GitHub MCP читает репы, PR и CI только в read-only режиме.</p></div></article><article class="architecture-step"><span>6</span><div><strong>План, очередь и итог</strong><p>Изменения собираются в один preview. После подтверждения план выполняется один раз, а финальный результат возвращается в Telegram.</p></div></article><article class="architecture-step"><span>7</span><div><strong>Фон без лишних токенов</strong><p>Мониторы сначала сравнивают хеш и diff. LLM вызывается только когда источник реально изменился.</p></div></article><article class="architecture-step"><span>8</span><div><strong>Код отдельно</strong><p>Задачи по коду уходят в изолированную очередь coding runner. Он возвращает причину, diff и тесты, но не деплоит сам.</p></div></article></div><div class="architecture-note"><strong>Что проверить сейчас</strong><p>В этом разделе выше виден живой статус Gateway, Trello, Calendar, GitHub, runner и backup.</p></div><div class="dialog-actions"><button class="primary" type="submit">Понятно</button></div></form></dialog>

...
Кабинет — отдельный маленький процесс рядом с Hermes. Первый экран показывает
один текущий фокус, ближайшие события и три главные задачи, а не дублирует
технический dashboard. Во вкладках можно просмотреть весь доступный список
Trello и ближайшие семь дней Google Calendar, открыть исходную карточку или
событие в их родном приложении, а также закрыть задачу, переместить её,
сменить приоритет или перенести событие. Вкладка `Код` показывает реальную
очередь sandbox coding/research jobs, их статусы, репозиторий и отчёт runner;
она не запускает код с сервера. Вкладка `Память` даёт поиск по SQLite FTS,
историю правок заметок и список явно сохранённых источников знаний.

Явная просьба разобрать или исправить код в Telegram создаёт только
подтверждаемый coding job: его можно поставить и из обычного режима `Быстро`,
но выполнить способен лишь отдельный Mac runner в Docker sandbox. VDS не
получает shell-доступ к репозиторию, не делает push, merge или deploy.
...
Кнопка `Добавить` создаёт задачу, встречу, напоминание или заметку. Любое
изменение Trello или Google Calendar сначала собирается в один preview-план;
внешние изменения применяются только после одной явной кнопки `Применить`.
Номер plan дополнительно привязан к Telegram-сессии создавшего его владельца.
Во вкладке `Память` можно добавить один публичный HTTPS URL прямо из Mini App:
сначала показывается preview, и только затем JarHert скачивает и сохраняет эту
одну страницу. В блоке `Радар` кнопка `Тише` задаёт quiet hours: изменения не
теряются, а собираются в один digest вместо ночного спама.

### Локальный голосовой inbox

Профиль включает бесплатное локальное распознавание Telegram-голосовых через
`faster-whisper` с моделью `small`. Оно работает на VPS, не использует API-ключ
и не отправляет аудио стороннему провайдеру. Сырая расшифровка не дублируется в
чате: одна голосовая даёт один нормальный ответ или один preview-план. Для
набора задач, заметок, обещаний и встреч достаточно одной кнопки
`Подтвердить всё`; JarHert не просит отдельное подтверждение на каждый пункт.
